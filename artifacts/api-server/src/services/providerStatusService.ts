...
let statusCache: { expiresAt: number; value: ProviderStatus[] } | null = null;
...
async function checkAllProviders(): Promise<ProviderStatus[]> {
  const onet =
  ...
  return Promise.all([
    ...
    check("osha", "OSHA", "public", true, () =>
      get("https://www.osha.gov/severe-injury-reports"),
    ),
...
export async function getProviderStatuses(
  ...
  if (!force && statusCache && statusCache.expiresAt > Date.now())
    return statusCache.value;
  if (inFlight) return inFlight;
  inFlight = checkAllProviders()
    .then((value) => {
      statusCache = { expiresAt: Date.now() + STATUS_TTL, value };
      return value;
    })
    .finally(() => {
      inFlight = null;
