...
async function json(url: string): Promise<JsonRecord> {
  ...
  if (body.trim()) {
    if (!contentType.toLowerCase().includes('application/json')) {
      throw new Error(`Expected JSON from ${url}, but received ${contentType || 'a non-JSON response'}.`);
    }
    try {
  ...
  if (!response.ok) {
    throw new Error(asString(payload.error, `Request failed (${response.status}).`));
  }
