...
function isGuidelineLike(value: unknown): value is Guideline {
  return isRecord(value) && typeof value.id === "string" && typeof value.sourceName === "string" && typeof value.agency === "string";
}

function isSourceLike(value: unknown): value is Source {
  return isRecord(value) && typeof value.id === "string" && typeof value.title === "string" && typeof value.organization === "string";
}

function sanitizeImportData(data: unknown): { guidelines: Guideline[]; sources: Source[] } | null {
...
export function useStore() {
  ...
  return { guidelines, sources, saveGuideline, deleteGuideline, saveSource, deleteSource, clearAll, exportAll, importAll };
}
