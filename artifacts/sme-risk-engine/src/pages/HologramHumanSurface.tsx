...
function addHead(out: CloudPoint[]) {
  ...
  // Very subtle facial landmark points; no hard robot-like stripes.
  const face = [
...
function torsoProfile(t: number) {
  // t=0 clavicle, t=1 pelvis top. Smooth chest, ribcage, waist, iliac flare.
  let width: number;
...
function addArms(out: CloudPoint[]) {
  for (const side of [-1, 1] as const) {
    ...
    // Hand is a flattened tapered continuation, not a separate ball.
    addSweep(out, [
...
function addLegs(out: CloudPoint[]) {
  for (const side of [-1, 1] as const) {
    ...
    // Feet project forward and taper toward toes.
    addSweep(out, [
...
export default function HologramHumanSurface({ view, tiltX, tiltY, activeRegion, regionScores }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      // Soft body aura: keeps the cloud luminous without turning it into solid neon plastic.
      ...
      // A restrained vertical scanner accent across the body.
