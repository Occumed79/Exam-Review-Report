...
function calcMAP(values: Record<string, string>): CalcResult | null {
  ...
  return {
    ...
    interpretation: 'Arithmetic estimate from systolic and diastolic pressure. Apply any program-specific BP criteria separately.',
...
function calcPackYears(values: Record<string, string>): CalcResult | null {
  ...
  if (Number.isNaN(cigarettesPerDay) || Number.isNaN(years) || cigarettesPerDay < 0 || years < 0) return null;
  const packYears = (cigarettesPerDay / 20) * years;
  return {
    label: 'Smoking exposure',
    value: `${packYears.toFixed(1)} pack-years`,
    interpretation: 'Exposure summary only. Screening or occupational criteria must come from the applicable current standard.',
...
function calcWalkingMETs(values: Record<string, string>): CalcResult | null {
  ...
  const vo2 = (0.1 * speedMetersPerMinute) + (1.8 * speedMetersPerMinute * grade) + 3.5;
  const mets = vo2 / 3.5;
  const speedNote = speedMph >= 1.9 && speedMph <= 3.7
...
const CALCULATORS: CalcDefinition[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'age', label: 'Age', unit: 'years', type: 'number', min: 18, max: 120, step: 1 },
  ...
  },
  {
    ...
    icon: Heart,
    fields: [
      { key: 'sbp', label: 'Systolic BP', unit: 'mmHg', type: 'number', min: 50, max: 300, step: 1 },
  ...
  },
  {
    ...
    icon: Cigarette,
    fields: [
      { key: 'cigarettes_per_day', label: 'Cigarettes per day', type: 'number', min: 0, max: 200, step: 1 },
      { key: 'years', label: 'Years smoked', type: 'number', min: 0, max: 100, step: 0.5 },
    ],
  ...
  },
  {
    ...
    icon: Zap,
    fields: [
      { key: 'speed_mph', label: 'Treadmill speed', unit: 'mph', type: 'number', min: 0.1, max: 8, step: 0.1 },
...
const REFERENCES = [
  { label: 'BMI — CDC adult BMI categories', url: 'https://www.cdc.gov/bmi/adult-calculator/bmi-categories.html' },
  { label: 'eGFR — National Kidney Foundation CKD-EPI 2021 guidance', url: 'https://www.kidney.org/recommendations-implementing-ckd-epi-2021-race-free-egfr-calculation-guidelines-clinical' },
  { label: 'Current cardiovascular risk — AHA PREVENT', url: 'https://professional.heart.org/en/guidelines-and-statements/about-prevent-calculator' },
];
...
export default function ClinicalCalculatorV3() {
  ...
  const missing = useMemo(() => active.fields.filter((field) => values[field.key] === undefined || values[field.key] === '').map((field) => field.label), [active, values]);
    ...
    <div className="clinical-workbench" data-testid="clinical-calculators">
      <header className="clinical-header">
        <div>
          ...
          <p>Transparent quick calculations only. Current cardiovascular risk assessment is routed to the official AHA PREVENT calculator rather than reimplemented here.</p>
        </div>
        <a
          href="https://professional.heart.org/en/guidelines-and-statements/about-prevent-calculator"
          target="_blank"
          rel="noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: '.45rem', padding: '.55rem .7rem', border: '1px solid rgba(180,215,208,.2)', borderRadius: 8, color: '#b4d7d0', textDecoration: 'none', fontSize: '.72rem', fontWeight: 700 }}
        >
      ...
      <div className="clinical-layout">
        ...
        <main className="clinical-panel" onKeyDown={(event) => { if (event.key === 'Enter' && missing.length === 0) calculate(); }}>
          <div className="clinical-panel-head">
            <div><span>ACTIVE CALCULATION</span><h2>{active.label}</h2><p>{active.description}</p></div>
            ...
            {active.fields.map((field) => (
              <label key={field.key} className="clinical-field">
                ...
                {field.type === 'select' ? (
                  ...
                  <input type="number" value={values[field.key] ?? ''} min={field.min} max={field.max} step={field.step} onChange={(event) => setValue(field.key, event.target.value)} placeholder="Enter value" />
                )}
              </label>
            ))}
          </div>
          ...
          {result && (
            <section className="clinical-result">
              <div><span>{result.label}</span><strong>{result.value}</strong><p>{result.interpretation}</p></div>
              <div className="clinical-result-reference"><Info size={13} /><span>{result.reference}</span></div>
            </section>
          )}

          <div className="clinical-disclaimer"><AlertTriangle size={13} /><span>Decision-support calculation only. Apply the current controlling medical or program standard separately; this tool does not produce clearance.</span></div>

          <section className="clinical-reference">
            <button onClick={() => setShowReferences((value) => !value)}><Info size={13} /> Current source links {showReferences ? <ChevronDown size={13} /> : <ChevronRight size={13} />}</button>
            {showReferences && <div>{REFERENCES.map((reference) => <p key={reference.label}><a href={reference.url} target="_blank" rel="noreferrer" style={{ color: '#b4d7d0', textDecoration: 'none' }}>{reference.label} <ExternalLink size={9} /></a></p>)}</div>}
          </section>
        </main>
      </div>
    </div>
