...
function buildRegionSignals(measured: OccupationInjuryEvidence | null, profile: OccupationalInjuryProfile): RegionSignal[] {
  ...
  return [...keys]
    .map((key) => {
      ...
      const derivedWeight = Math.min(0.78, 0.34 + derived.length * 0.11);
      return {
        key,
        label: REGION_LABELS[key],
        score: derivedWeight,
...
function BodyFigure({
  ...
  const shape = (key: RegionKey, node: React.ReactNode) => (
    <g
      key={`${view}-${key}`}
      className={`anatomy-region${active === key ? ' active' : ''}`}
  ...
  return (
    <div className="anatomy-figure-wrap">
      ...
      <svg className="anatomy-figure" viewBox="0 0 180 430" aria-label={`${view} body injury map`}>
        <defs>
          <filter id={`softGlow-${view}`} x="-80%" y="-80%" width="260%" height="260%">
        ...
        <g className="anatomy-silhouette">
          ...
          <path d="M56 92 Q39 110 33 145 L20 240" />
          <path d="M124 92 Q141 110 147 145 L160 240" />
          <path d="M70 188 L63 252 L58 377" />
          <path d="M110 188 L117 252 L122 377" />
        ...
        {shape('upperExtremity', <><path d="M51 99 Q38 120 33 151 L23 229" /><path d="M129 99 Q142 120 147 151 L157 229" /></>)}
        ...
        {shape('lowerExtremity', <><path d="M66 205 L62 271 L58 371" /><path d="M114 205 L118 271 L122 371" /></>)}
...
export default function InjuryBodyMap({
  ...
  profile: OccupationalInjuryProfile;
}) {
    ...
    <section className="injury-anatomy-shell">
      <div className="injury-anatomy-stage">
        ...
        <div className="injury-anatomy-visual">
          ...
          <BodyFigure view="front" signals={signalMap} active={active} onSelect={setActive} />
          <BodyFigure view="back" signals={signalMap} active={active} onSelect={setActive} />
        </div>
      </div>
