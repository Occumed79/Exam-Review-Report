...
function classifyRegion(yNorm: number, xNorm: number): RegionKey {
...
function applyRegionColors(state: RenderState | null, activeRegion: RegionKey | null, regionScores: Partial<Record<RegionKey, number>>) {
  if (!state) return;
  const colorAttribute = state.geometry.getAttribute('color');
  if (!colorAttribute) return;
  const colors = colorAttribute.array as Float32Array;
  const base: [number, number, number] = [0.40, 0.92, 1.00];

  for (let i = 0; i < state.regions.length; i += 1) {
    const region = state.regions[i];
    const score = regionScores[region] ?? 0;
    const isActive = activeRegion === region;
    const target = isActive ? heatColor(score) : base;
    const scoreLift = !isActive && score > 0 ? Math.min(score * 0.08, 0.07) : 0;
    const offset = i * 3;
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        const scale = 3.80 / humanHeight;
        const positions = new Float32Array(POINT_COUNT * 3);
        const colors = new Float32Array(POINT_COUNT * 3);
        const regions: RegionKey[] = new Array(POINT_COUNT);
        ...
        applyRegionColors(state, propsRef.current.activeRegion, propsRef.current.regionScores);
        resize();
        ...
        console.error('CC0 hologram mesh failed to load', error);
