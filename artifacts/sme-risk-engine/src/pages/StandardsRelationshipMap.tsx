...
export default function StandardsRelationshipMap({ frameworks, findings, activeFindingId, onFindingSelect }: Props) {
      ...
      try {
        ...
        findings.slice(0, 14).forEach((finding) => {
          elements.push({
            data: {
              ...
              label: finding.title.length > 34 ? `${finding.title.slice(0, 31)}…` : finding.title,
              type: 'finding',
              level: finding.level,
          ...
          elements.push({
            data: {
              id: `edge:${finding.standardId}:${finding.id}`,
              source: `std:${finding.standardId}`,
        ...
        cy = cytoscape({
          ...
          style: [
            ...
            {
              selector: 'edge[type = "sourceEdge"]',
              style: {
                width: 1.5,
                'line-style': 'dashed',
                'line-dash-pattern': [5, 7],
                'line-color': '#4edff5',
                'target-arrow-color': '#4edff5',
                opacity: 0.52,
              },
            },
  ...
  }, [frameworks, findings, onFindingSelect]);
