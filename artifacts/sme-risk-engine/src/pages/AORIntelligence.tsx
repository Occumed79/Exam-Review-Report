...
export default function AORIntelligence() {
  ...
  useEffect(() => {
    ...
    setPartialNews([]);
    searchNewsIntelligence(selected.newsQuery, undefined, controller.signal)
      .then((result) => {
        if (!active) return;
    ...
    Promise.allSettled(
      selected.healthCountries.map((country) =>
        fetchWhoIndicators(country.iso3, [], controller.signal),
      ),
    ).then((results) => {
      if (!active) return;
    ...
    searchCongressIntelligence(selected.policyQuery, 12, controller.signal)
      .then(
        (result) =>
          active &&
  ...
  const failedSources = [news, health, congress, regulations].filter(
  ...
  const liveLabel = loading
    ...
    : failedSources === 4
      ...
      : failedSources > 0 || partialNews.length > 0
        ? "Live sources degraded"
  ...
  function capture(item: TimelineItem) {
    ...
    saveSource({
      ...
      url: item.url,
      publicationDate: item.date.slice(0, 10),
      lastReviewed: new Date().toISOString().slice(0, 10),
      reviewedBy: "",
      summary: `${category} saved from ${selected.abbreviation} intelligence.`,
      relevantConditions: "",
      relevantJobs: "Deployment / travel",
      relevantCountries: selected.countriesAreas.join(", "),
      sourceReliability: item.kind === "news" ? "Unverified" : "High",
      sourceCategory: category,
      notes: `Provider: ${item.provider}`,
  ...
  return (
    <div className="aor-command" data-testid="aor-intelligence">
      <header className="workstation-header">
        ...
        <span
          className={`status-pill ${loading ? "is-loading" : failedSources || partialNews.length ? "is-degraded" : "is-live"}`}
        >
          <i />
          {liveLabel}
        </span>
              ...
              <div className="health-grid">
                {health.data.slice(0, 9).map((item, index) => (
                  ...
                  >
                    ...
                    <a href="https://ghoapi.azureedge.net/api" target="_blank" rel="noreferrer">
                      WHO Global Health Observatory <ExternalLink size={10} />
                    </a>
                  </article>
                ))}
              </div>
